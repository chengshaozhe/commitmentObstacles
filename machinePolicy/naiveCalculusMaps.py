...
def grid_reward(s, a, sn, env=None, const=-1, terminals=None):
    actionCost = const * np.linalg.norm(a)
    if sn in terminals:
        return actionCost + sum(map(lambda f: env.features[f][sn], env.features))
    else:
        return actionCost + sum(map(lambda f: env.features[f][s], env.features))
...
class RunVI:
    ...
    def __call__(self, goalStates, obstacles_states):
        ...
        subGoalAValue = {(0, 5): 0}
        subGoalBValue = {(6, 5): 0}
        costAArea = [(0, 1), (0, 2), (0, 3)]
        costBArea = [(6, 1), (6, 2), (6, 3)]
        costAValue = {s: -1 for s in costAArea}
        costBValue = {s: -50 for s in costBArea}

        env.add_feature_map('subGoalA', subGoalAValue)
        env.add_feature_map('subGoalB', subGoalBValue)
        env.add_feature_map('costA', costAValue)
        env.add_feature_map('costB', costBValue)
        ...
        const = -1
        reward_func = ft.partial(grid_reward, env=env, const=const, terminals=goalStates)
        ...
        # print(Q_dict)
        Q_dict_output = {(s, goalStates): {a: Q[si, ai] for (ai, a) in enumerate(A)} for (si, s) in enumerate(S)}
        return Q_dict_output
...
if __name__ == '__main__':
    ...
    obstacles_states = [(1, 1), (1, 2),(1,3),(2,1),(2,2),(2,3),(3,1),(3,2),(3,3),(4,1),(4,2),(4,3),(5,1),(5,2),(5,3)]
    # obstacles_states = []
    Q_dict = runVI(goalStates, obstacles_states)
    print(Q_dict[(4, 0), goalStates])
