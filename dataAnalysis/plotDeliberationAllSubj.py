...
if __name__ == '__main__':
    ...
    # participants = ['human', 'intentionModel/threshold0.12infoScale8']
    participants = ['human0331']
    # participants = ['intentionModel2/threshold0.04infoScale9']
    ...
    # df["trialType"] = ['First half' if index > 144 else 'Second half' for index in df.index]

    # df["trajLength"] = df.apply(lambda x: len(eval(x['trajectory'])), axis=1)
    # df = df[df["trajLength"] > 10]
    ...
    statDF["meanReactionTime"] = dfExpTrail.groupby(['name', 'decisionSteps'])['totalTime'].mean()
...
# binomial test
    crosstab, res = researchpy.crosstab(dfExpTrail['hasAvoidPoint'], dfExpTrail['decisionSteps'], test="chi-square")
    print(crosstab)
    print(res)

    from scipy.stats import binom_test
    p = binom_test(154, 240, alternative='greater')
    print(p)
    ...
    # ax = sns.barplot(x="decisionSteps", y="ShowCommitmentPercent", hue="name", data=statDF, ci=None, palette=colorList)


    # ax = sns.barplot(x="decisionSteps", y="ShowCommitmentPercent", hue="meanReactionTime", data=statDF, ci=95)

    # ax = sns.barplot(x="decisionSteps", y="ShowCommitmentPercent", hue="name", data=statDF, ci=None)

    ax = sns.barplot(x="decisionSteps", y="ShowCommitmentPercent", hue="participantsType", data=statDF, ci=95)
